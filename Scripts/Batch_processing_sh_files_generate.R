...
setwd(path_raw)
script_test<-read_file("joint-call.sh")

#get the interval list
intervals<- read.table("intervals.list")

#set the output folder
path_out<-paste0(path_raw,"/joint-call_sh_output")
setwd(path_out)
sh_file_namelist <- c()
...
for (i in 1:times){
  ...
  write_file(script_out,paste0("joint-call-",intervals[i,],".sh"))
  #get the sh file's namelist
  sh_file_namelist <- c(sh_file_namelist,paste0("joint-call-",intervals[i,],".sh"))
  rm(script_out)
}
#perform sh bulk
rm(list=ls())
